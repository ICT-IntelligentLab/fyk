...
X = qammod(X_i, M, "UnitAveragePower", true);  % QAM
...
normDelays   = round(pathDelays * samplingFreq);          % 延迟几个采样点
normDopplers = N * pathDopplers / samplingFreq;           % 归一化多普勒 ν

% 打印每条路径的详细信息
fprintf('\n');
fprintf('============================================================\n');
fprintf('              每条路径详细信息（共 %d 条）\n', P);
fprintf('============================================================\n');
fprintf('%4s %12s %14s %18s %14s\n', ...
        '路径', '时延(采样点)', '时延(μs)', '多普勒(Hz)', '归一化多普勒 ν');
fprintf('%s\n', repmat('-',1,75));
for p = 1:P
    delay_us = pathDelays(p)*1e6;
    fprintf('%4d %12d %14.2f %16.1f %18.6f\n', ...
        p, normDelays(p), delay_us, pathDopplers(p), normDopplers(p));
end
fprintf('c1_AFDM = %.10f   (guardWidth = 4，保险系数)\n', c1_AFDM);
fprintf('============================================================\n\n');
